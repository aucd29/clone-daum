...
abstract class BaseActivity<T : ViewDataBinding, M: ViewModel>
    ...
    protected val viewModelClass = Reflect.classType<Class<M>>(this, 1)
    protected val viewModel: M by lazy(LazyThreadSafetyMode.NONE) { initViewModel() }

    private val dp = CompositeDisposable()
    private lateinit var backPressed: BackPressedManager
...
//    }
    ...
    protected open fun bindViewModel() {
        Reflect.method(binding, SET_VIEW_MODEL, Reflect.Params(viewModelClass, viewModel))
    ...
    protected open fun initBackPressed() {
        backPressed = BackPressedManager(this, binding.root)
    }
    ...
    override val commandEventModels: ArrayList<ICommandEventAware> = arrayListOf()
    override fun disposable() = dp
    override fun activity() = this@BaseActivity
    override fun rootView() = binding.root
}
...
abstract class BaseFragment<T: ViewDataBinding, M: ViewModel>
    ...
    @ViewModelScope
    protected var viewModelScope = SCOPE_FRAGMENT
    protected val viewModelClass = Reflect.classType<Class<M>>(this, 1)
    protected val viewModel: M by lazy(LazyThreadSafetyMode.NONE) { initViewModel() }
    protected var stateViewModelFactory: AbstractSavedStateViewModelFactory? = null
...
//    private var mOnBackPressedCallback: OnBackPressedCallback? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        binding = dataBinding(layoutId, container)
...
abstract class BaseDialogFragment<T: ViewDataBinding, M: ViewModel> constructor()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        binding = dataBinding(layoutId, container, false)
        binding.root.isClickable = true
...
abstract class BaseBottomSheetDialogFragment<T: ViewDataBinding, M: ViewModel> constructor()
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(BaseBottomSheetDialogFragment::class.java)
    }

    protected val dp = CompositeDisposable()
    ...
    private fun stateCallback() {
        binding.root.globalLayoutListener {
                    ...
                    override fun onStateChanged(view: View, state: Int) {
                        ...
                        when (state) {
                            ...
                            BottomSheetBehavior.STATE_HIDDEN -> {
                                if (logger.isDebugEnabled) {
                                    logger.debug("STATE HIDDEN | STATE_COLLAPSED")
                                }
...
interface BaseEventAware {
    val commandEventModels: ArrayList<ICommandEventAware>
    ...
    fun addCommandEventModel(viewmodel: ViewModel) {
        if (viewmodel is ICommandEventAware) { commandEventModels.add(viewmodel) }
    ...
    fun removeCommandEventModel(viewmodel: ViewModel) {
        if (viewmodel is ICommandEventAware) {
            commandEventModels.remove(viewmodel)
        }
        if (viewmodel is LifecycleObserver) {
